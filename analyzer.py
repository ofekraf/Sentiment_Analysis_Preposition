...
class Analyzer:
    ...
    def plot(self):
        ...
        for mod_idx, module in enumerate(self.sentiment_modules):
            for prop_idx, file_name in enumerate(self.list_of_files):
                all_plots[mod_idx, prop_idx].scatter(self.coords[file_name][module][0], self.coords[file_name][module][1])
    ...
    def analyze(self):
        ...
        for file in self.list_of_files:
            self.coords[file] = {n:([],[]) for n in self.sentiment_modules.keys()}
            with open(file, 'r') as reader:
                for line in reader:
                    ...
                    for mod_name, module in self.sentiment_modules.items():
                        self.coords[file][mod_name][0].append(self.calc_score(module, first))
                        self.coords[file][mod_name][1].append(self.calc_score(module, sec))
    ...
    def initialize_modules(self):
        ...
        self.sentiment_modules["TextBlob"] = (lambda x:TextBlob(x).sentiment,0,lambda x: x*5)
